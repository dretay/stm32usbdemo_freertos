...
static int8_t stm32usbdemo_Init(void);
static int8_t stm32usbdemo_DeInit(void);
static int8_t stm32usbdemo_Control(uint8_t cmd, uint8_t* pbuf, uint16_t length);
static int8_t stm32usbdemo_Receive(uint8_t* pbuf, uint32_t *Len);

USBD_CDC_ItfTypeDef USBD_CDC_stm32usbdemo_fops = 
{
	stm32usbdemo_Init,
	stm32usbdemo_DeInit,
	stm32usbdemo_Control,
	stm32usbdemo_Receive
};
...
/**
  ...
  * @retval Result of the operation: USBD_OK if all operations are OK else USBD_FAIL
  */
static int8_t stm32usbdemo_Receive(uint8_t* Buf, uint32_t *Len) {
